...
class history_sales extends REST_Controller
{
    ...
    {
        $getSalesHistory = $this->history->getSalesHistory();

        if ($getSalesHistory) {
            $this->response([
                'status' => true,
                'message' => 'Load History Successfully',
                'rownum_history' => $getSalesHistory->num_rows(),
                'result_history' => $getSalesHistory->result_array()
    ...
    public function index_post()
    {
        if ($this->post()) {
            ...
            if (trim($sale_id) == "") {
                ...
                $getDetails = $this->history->salesHistoryDetail($sale_id);
